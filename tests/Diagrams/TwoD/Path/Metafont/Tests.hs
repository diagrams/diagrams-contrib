...
module Diagrams.TwoD.Path.Metafont.Tests where
...
illustrateSegment (FLinear from to) =
  position
    [ (from, ptMark # fc blue)
    , (to, ptMark # fc blue)
    ]
illustrateSegment (FCubic from z1 z2 to) =
  position
    [ (z1, ptMark # fc red)
    , (z2, ptMark # fc red)
    ]
...
refPts = position $ zip (map p2 $ (,) <$> [0, 1, 2] <*> [0, 1]) (repeat c)
 where
