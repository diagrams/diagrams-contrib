...
reverseSeg s = MFS (s ^. x2) (PJ (rDir $ s ^. pj . d2) (s ^. pj . j . to rj) (rDir $ s ^. pj . d1)) (s ^. x1)
 where
...
controlPtDirs s@(MFS z0 (PJ _ jj@(Right (CJ u v)) _) z1) = s & pj .~ dirs
 where
  dirs = PJ (dir z0 u) jj (dir v z1)
  dir :: P2 n -> P2 n -> Maybe (PathDir n)
  dir p0 p1
    | p0 == p1 = Just $ PathDirCurl 1
    | otherwise = Just . PathDirDir . direction $ (p1 .-. p0)
controlPtDirs s = s
...
groupSegments [] = []
groupSegments (s : ss) = (s : open) : groupSegments rest
 where
...
solvePath (MFP False ss) = MFP False (concatMap solveLine . groupSegments $ ss)
-- A simple loop.  All directions are unknown, curvature gives us enough equations.
solvePath (MFP True ss@(s1 : s2 : s3)) | all (view $ pj . d1 . to isNothing) ss = MFP True . NE.toList . solveLoop $ s1 :|| s2 :| s3
solvePath (MFP True ss) = MFP True ss''
 where
  ss' = groupSegments ss
  ss'' = concatMap solveLine $ case ss' ^?! _head ^?! _head . pj . d1 of
    (Just (PathDirDir _)) -> ss'
    _ -> fromMaybe [] (ss' ^? _tail . _init) ++ [last ss' ++ head ss']
...
solveLoop :: forall n. RealFloat n => NonSingleton (MFS n) -> NonEmpty (MetafontSegment (Dir n) (BasicJoin n) n)
solveLoop ss = zipWith3NE setDirs (NS.toNonEmpty ss) thetas phis
 where
  segmentPairs = NS.zipWith (,) ss (rotateNS ss)
  thetas, phis :: NonEmpty n
  thetas = loopDirs ss
  phis = negate <$> NE.zipWith (+) (NS.toNonEmpty $ psi <$> segmentPairs) (rotateNE thetas)

zipWith3NE :: (a -> b -> c -> d) -> NonEmpty a -> NonEmpty b -> NonEmpty c -> NonEmpty d
zipWith3NE f (a :| as) (b :| bs) (c :| cs) = f a b c :| zipWith3 f as bs cs
...
loopDirs :: RealFloat n => NonSingleton (MFS n) -> NonEmpty n
loopDirs ss = solveCyclicTriDiagonal lower diag upper products ll ur
 where
...
loopEqs ::
  RealFloat n =>
  NonSingleton (MFS n) ->
  (NonEmpty n, NonEmpty n, NonEmpty n, NonEmpty n, n, n)
loopEqs ss = (lower, NS.toNonEmpty diag, upper, NS.toNonEmpty products, ll, ur)
 where
  lower = fmap aCo (initNS ss)
  sLast = lastNS ss
  diag = NS.zipWith (+) (bCo <$> consNS sLast ss) (fmap cCo ss)
  upper = fmap dCo (initNS ss)
  ur = aCo sLast
  ll = dCo sLast
  segmentPairs = NS.zipWith (,) (lastNS ss :|| initNS ss) ss
  products =
    NS.zipWith
      (-)
      (fmap (\s@(l, _) -> (-1) * bCo l * psi s) segmentPairs)
      ( snocNS
          ( NE.zipWith
              (*)
              (dCo <$> NS.toNonEmpty ss)
              (psi <$> NS.tail segmentPairs)
          )
          (dCo sLast * psi (headNS segmentPairs))
      )
...
solveLine ss = zipWith3 setDirs ss (init thetas) phis
 where
...
setDirs ::
  ...
  MFS n ->
  -- | theta, the offset angle at the starting point
  n ->
  -- | phi, the ofset angle at the endpoint
  n ->
  MetafontSegment (Dir n) (BasicJoin n) n
setDirs (MFS z0 (PJ w0' jj w1') z1) t p = MFS z0 (PJ w0 jj w1) z1
 where
...
-- NonEmpty -> NonEmpty ?
lineDirs (s1 : s2 : ss) = NE.toList $ solveTriDiagonal (NS.toNonEmpty lower) (NS.toNonEmpty diag) (NS.toNonEmpty upper) (NS.toNonEmpty products)
 where
  (lower, diag, upper, products) = lineEqs (s1 :|| (s2 :| ss))
lineDirs [] = []
lineDirs [s] | leftCurl s && rightCurl s = [0, 0]
lineDirs [s] | rightCurl s = NE.toList $ solveTriDiagonal (a :| []) (1 :| [c]) (0 :| []) (normalizeTurns t :| [r])
 where
  (a, c, r) = solveOneSeg s
...
consNS :: a -> NonSingleton a -> NonSingleton a
consNS x (y :|| ys) = x :|| NE.cons y ys

initNS :: NonSingleton a -> NonEmpty a
initNS (x :|| xs) = x :| NE.init xs

snocNE :: [a] -> a -> NonEmpty a
snocNE [] a = a :| []
snocNE (x : xs) a = x :| (xs ++ [a])

snocNS :: NonEmpty a -> a -> NonSingleton a
snocNS (x :| xs) y = x :|| snocNE xs y

headNS :: NonSingleton a -> a
headNS (a :|| _) = a

sndNS :: NonSingleton a -> a
sndNS (_ :|| (a :| _)) = a

lastNS :: NonSingleton a -> a
lastNS (_ :|| as) = NE.last as

rotateNS :: NonSingleton a -> NonSingleton a
rotateNS s = snocNS (NS.tail s) (headNS s)

rotateNE :: NonEmpty a -> NonEmpty a
rotateNE s = snocNE (NE.tail s) (NE.head s)
...
-- lineDirs.
lineEqs :: RealFloat n => NonSingleton (MFS n) -> (NonSingleton n, NonSingleton n, NonSingleton n, NonSingleton n)
lineEqs ss = (lower, diag, upper, products)
 where
  segmentPairs = NE.zip (initNS ss) (NS.tail ss)
  lower = snocNS (fmap aCo (initNS ss)) an
  diag = c0 :|| snocNE (NE.toList $ NE.zipWith (+) (fmap bCo (initNS ss)) (fmap cCo (NS.tail ss))) cn
  upper = d0 :|| fmap dCo (NS.tail ss)
  products =
    r0
      :|| snocNE
        ( NE.toList
            ( NE.zipWith
                (-)
                (fmap (\s@(l, _) -> -(bCo l * psi s)) segmentPairs)
                ( NE.zipWith
                    (*)
                    (fmap dCo (NS.tail ss))
                    (snocNE (fmap psi (NE.tail segmentPairs)) 0)
                )
            )
        )
        rn
  (d0, c0, _) = solveOneSeg . reverseSeg $ s0
  r0 = r0' (s0 ^. pj . d1 . to fromJust)
   where
    r0' (PathDirDir d) = normalizeTurns t
     where
      t = view turn $ angleBetweenDirs d (direction $ s0 ^. x2 .-. s0 ^. x1)
    r0' (PathDirCurl _) = negate $ d0 * psi (s0, sndNS ss)
  s0 = headNS ss
  (an, cn, rn) = solveOneSeg (lastNS ss)
...
solveOneSeg s = (a, c, r)
 where
  a = a' (s ^. pj . d2 . to fromJust)
   where
    a' (PathDirDir _) = 0
    a' (PathDirCurl g) = (3 - beta s) * (beta s) ** 2 * g / (alpha s ** 2) + alpha s
  c = c' (s ^. pj . d2 . to fromJust)
   where
    c' (PathDirDir _) = 1
    c' (PathDirCurl g) = beta s ** 3 * g / (alpha s ** 2) + 3 - alpha s
  r = r' (s ^. pj . d2 . to fromJust)
   where
    r' (PathDirDir d) = normalizeTurns t
     where
...
computeControls (MFS z0 (PJ w0 (Left (TJ a b)) w1) z1) =
 ...
 where
  ...
  sinR = sin . view rad
  boundingTriangleExists =
    signum (sinR theta) == signum (sinR phi)
...
hobbyF theta' phi' =
  let theta = theta' ^. rad
      phi = phi' ^. rad
   in (2 + sqrt 2 * (sin theta - sin phi / 16) * (sin phi - sin theta / 16) * (cos theta - cos phi))
...
locatedTrail (MFP False ss) =
  (wrapLine . fromSegments . map importSegment $ ss)
    `at` (head ss ^. x1)
locatedTrail (MFP True ss) =
  (wrapLoop . fromSegments . map importSegment $ ss)
    `at` (head ss ^. x1)
